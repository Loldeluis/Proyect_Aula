...
require_once($baseDir . '/Model/crud/EjercicioCrud.php'); // ✅ Agregamos clase de ejercicios
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        if ($crud->crearUsuario($usuario)) {
            ...
            // ✅ Crear ejercicios iniciales al registrar usuario
            $ejercicioCrud = new EjercicioCrud();
            $cedula = $_POST['cedula'];
            if (!$ejercicioCrud->crearEjerciciosUsuario($cedula)) {
                error_log("⚠️ No se pudieron crear los ejercicios para el usuario $cedula.");
                throw new Exception("Usuario registrado, pero hubo un problema al generar sus ejercicios.");
            }
